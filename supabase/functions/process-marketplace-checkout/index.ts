...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Créer les commandes ET les escrow dans escrow_payments (TABLE CANONIQUE)
    const orderIds: string[] = [];
    
    for (const item of cartItems) {
      const orderTotal = item.price * item.quantity;

      // Créer la commande avec adresse et téléphone
      ...
      // ✅ CORRECTION: Créer l'escrow dans escrow_payments (pas escrow_transactions)
      ...
      if (escrowError) {
        ...
        // CRITICAL: Ne pas continuer si escrow échoue
        throw new Error(`Échec création escrow: ${escrowError.message}`);
      }

      console.log(`✅ Escrow (escrow_payments) created for order: ${order.id}`);
