  ...
  const { data: subscription, isLoading, refetch } = useQuery({
    ...
    queryFn: async (): Promise<DriverSubscription> => {
      ...
      return {
        ...
        features: getFeatures(tier, serviceType, plan?.commission_rate || 10)
      };
    },
    enabled: !!user,
    staleTime: 5 * 60 * 1000,
...
function getDefaultSubscription(serviceType: 'taxi' | 'delivery'): DriverSubscription {
  return {
    ...
    plan: 'free',
    planName: 'Gratuit',
    planLabel: getPlanLabel('free', serviceType),
    expiresAt: null,
    ridesRemaining: serviceType === 'taxi' ? 20 : 30,
    ridesLimit: serviceType === 'taxi' ? 20 : 30,
    commissionRate: 10,
    isActive: true,
    features: ['10% commission', serviceType === 'taxi' ? '20 courses/jour' : '30 livraisons/jour']
  };
}
