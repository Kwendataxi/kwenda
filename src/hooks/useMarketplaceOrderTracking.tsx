...
export function useMarketplaceOrderTracking({
  ...
  const loadInitialData = useCallback(async () => {
    try {
      ...
      // ÉTAPE 2 : Si livraison Kwenda, chercher l'assignation de chauffeur
      let driverLocation = null;
      let driverProfile = null;

      if (orderData.vendor_delivery_method === 'kwenda' && enableTracking) {
        console.log('🚚 Livraison Kwenda détectée, recherche du chauffeur...');
  ...
  const setupRealtimeSubscriptions = useCallback(() => {
    ...
    // 2. Écouter la position du chauffeur (si livraison Kwenda)
