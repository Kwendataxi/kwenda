...
const TOTAL_STEPS = 10; // Nombre d'étapes visuelles
...
export const useScratchProgress = (): UseScratchProgressReturn => {
  ...
  const calculateSteps = (points: number): ProgressStep[] => {
    ...
    return Array.from({ length: TOTAL_STEPS }, (_, i) => {
      const hasReward = i === 4 || i === TOTAL_STEPS - 1; // Récompense à 50% et 100%
      return {
        position: i,
        completed: i < currentStep,
        isCurrent: i === currentStep,
        reward: hasReward ? (i === TOTAL_STEPS - 1 ? 'card' : 'gift') : undefined
      };
    });
  };
