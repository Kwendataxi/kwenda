...
export const RideActionPanel: React.FC = () => {
  ...
  const handleAccept = async () => {
    ...
    // Vérifier si le chauffeur a un abonnement actif (optionnel)
    ...
    if (hasActiveSubscription) {
      ...
      // Mode commission: 12% Kwenda + partenaire prélevés à la fin
      console.log('💰 Mode commission activé');
      toast.info('Course acceptée (Commission)', {
        description: 'Les frais seront prélevés à la fin de la course'
      });
    }
  ...
  const handleDriverCancel = async (reason: string, cancellationType: string) => {
    ...
    // Enregistrer dans l'historique des annulations
  ...
  const handleCompleteRide = async () => {
    if (!ride?.rideType || !user?.id) return;

    setLoading(true);

    try {
      // Appeler l'Edge Function qui gère commission + abonnement
      ...
      if (fnError) {
        console.error('❌ Erreur commission:', fnError);
        toast.error('Erreur lors du calcul de la commission');
        setLoading(false);
        return;
      }

      // Afficher le détail selon le mode de facturation
      if (data?.success) {
        if (data.billing_mode === 'subscription') {
          toast.success('🎉 Course terminée !', {
            description: `Mode abonnement - Courses restantes: ${data.rides_remaining ?? 'N/A'}`
          });
        } else {
          toast.success('🎉 Course terminée !', {
        ...
        toast.warning('Course terminée avec avertissement', {
          description: data?.message || 'Commission en attente'
        });
      }
  ...
  return (
    <>
      ...
      {/* Dialog d'annulation avec motifs */}
      {ride && (
        <CancellationDialog
