...
export const LocationDetailsSheet: React.FC<LocationDetailsSheetProps> = ({
  ...
  // 🚀 OPTIMISÉ: Chargement parallèle API key + Google Maps
  useEffect(() => {
    if (!open) return;
  ...
  // 🗺️ Initialiser la carte (SANS setTimeout)
  useEffect(() => {
  ...
  // 🔄 NOUVEAU: Watch position et mise à jour automatique si >50m
  useEffect(() => {
  ...
  const handleEditLocation = () => {
    ...
    if (!isEditing) {
      toast({ title: "📍 Mode édition activé", description: "Déplacez le marqueur" });
            ...
            <div className="relative w-full h-56 rounded-2xl overflow-hidden bg-muted/30 border border-border/30">
              <AnimatePresence mode="wait">
                {!mapLoaded ? (
                  ...
                  >
                    {/* Skeleton moderne au lieu de l'icône qui tourne */}
